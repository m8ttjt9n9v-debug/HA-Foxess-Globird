...
| Malformed session storage loses ownership evidence | `test_missing_session_evidence_holds_forced_mode_until_self_use`; `test_malformed_session_evidence_is_not_overwritten_while_forced`; `test_valid_idle_evidence_treats_unowned_forced_mode_as_external`; [safety decision](malformed-session-storage.md) | System | Add the separately versioned previous-valid-snapshot envelope only after rollback representation is approved |
